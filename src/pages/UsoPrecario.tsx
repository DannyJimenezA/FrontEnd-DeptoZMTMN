...
//import "../styles/PrecarioConcesion.css"; // Asegúrate de que la hoja de estilos esté correctamente importada
import { useForm } from "react-hook-form";
import { FaRegFileAlt } from "react-icons/fa";
import { useNavigate } from 'react-router-dom'; // Importa useNavigate para la redirección

function UsoPrecario() {
  ...
  const onSubmit = async () => {
    ...
    if (selectedFiles && selectedFiles.length > 0) {
      ...
      console.error('No se ha seleccionado ningún archivo de solicitud.');
      return;
    }
    ...
    try {
      const response = await fetch('http://localhost:3000/Precario', {
        method: 'POST',
        headers: {
