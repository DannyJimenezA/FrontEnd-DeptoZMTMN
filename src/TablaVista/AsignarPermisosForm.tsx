...
const AsignarPermisosForm: React.FC<AsignarPermisosFormProps> = ({ rol, onCancelar }) => {
  ...
  const [permisosSeleccionados, setPermisosSeleccionados] = useState<Permission[]>(rol.permissions);
  const [recursoActivo, setRecursoActivo] = useState<string>('');

  const location = useLocation(); // 👈 Obtenemos la ubicación actual

  // ✅ Cleanup si cambia de ruta
  useEffect(() => {
    return () => {
      onCancelar(); // Desmontar el formulario si cambia la ruta
    };
  }, [location.pathname, onCancelar]);
  ...
  return (
    <div className="max-w-4xl mx-auto bg-white shadow-md rounded-lg overflow-hidden">
      ...
      <div className="p-6">
        ...
        <div className="space-y-4">
          {permisos
            .filter((permiso) => permiso.resource === recursoActivo)
            .map((permiso) => {
              const isSelected = permisosSeleccionados.some((p) => p.id === permiso.id);
              return (
                <div
